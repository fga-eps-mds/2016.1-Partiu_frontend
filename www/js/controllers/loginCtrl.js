...
.controller('loginCtrl', function($scope, $ionicModal, $state, $ionicHistory, Profile, $http) {
  ...
  $scope.loginCallback = function(error, authData) {
    if (error) {
      ...
      var requestData = {"user": {"name": data.displayName,"email": data.email,"facebook_id": data.id,"photo_url": data.profileImageURL,
        "token": data.accessToken,"gender": data.cachedUserProfile.gender, "link_profile": data.cachedUserProfile.link }
      };
